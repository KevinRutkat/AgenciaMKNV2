...
export default function ViviendaCard({ vivienda, images, onEdit, onDelete }: ViviendaCardProps) {
  ...
  const getCategoryBadge = () => {
    if (vivienda.category === 'Usada') return { label: used, color: 'bg-blue-500' }
    if (vivienda.category === 'Sin estrenar') return { label: brandNew, color: 'bg-primary-green' }
    if (vivienda.category === 'Otro') return { label: other, color: 'bg-primary-blue' }
    // Si es alquiler, se mostrará con el badge específico de alquiler más abajo
    if (vivienda.is_rent || 
        (vivienda.category && vivienda.category.toLowerCase().includes('alquiler')) ||
        (vivienda.property_type && vivienda.property_type.toLowerCase().includes('alquiler')) ||
  ...
  // Mostrar el precio exactamente como viene de la base de datos (sin reformato)
  const displayPrice = (raw: string | null | undefined) => {
    if (!raw) return ''
    // Quitar todos los espacios (incluyendo múltiples o no-break spaces)
    const cleaned = raw.replace(/\s+/g, '').replace(/\u00A0/g, '')
    // Si ya termina en € (con o sin espacio previo) lo dejamos
    if (/€$/.test(cleaned)) return cleaned
    return `${cleaned}€`
  }

  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:scale-105 hover:-translate-y-1 h-full flex flex-col relative group">
      ...
      >
        ...
        {/* Precio overlay destacado - Solo visible en móviles */}
        ...
        <div className="absolute top-1 sm:top-2 right-1 sm:right-2 z-30 pointer-events-none">
          <span className={`${getCategoryBadge().color} text-white text-xs px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-full font-semibold shadow-md`}>
      ...
      <div className="p-3 sm:p-4 flex-1 flex flex-col">
        ...
        <h3 
          onClick={handleClick}
          className="text-base sm:text-lg font-semibold text-neutral-dark mb-2 line-clamp-1 cursor-pointer hover-text-primary-blue transition-colors"
        >
          {vivienda.name}
        </h3>
        ...
        {/* Detalles: m2, habitaciones, baños */}
        <div className="flex flex-wrap gap-3 md:gap-4 mb-4 text-sm text-gray-600">
          <div className="flex items-center gap-1.5" title={vivienda.metros ? `${vivienda.metros} m²` : 'N/A'}>
            <span className="text-sm">📐</span>
            <span className="font-medium">{vivienda.metros ? `${vivienda.metros} m²` : 'N/A'}</span>
          </div>
          <div className="flex items-center gap-1.5" title={vivienda.habitaciones ? `${vivienda.habitaciones} ${bedrooms}` : 'N/A'}>
            <span className="text-sm">🛏️</span>
            <span className="font-medium">{vivienda.habitaciones ? `${vivienda.habitaciones} ${bedrooms}` : 'N/A'}</span>
          </div>
          <div className="flex items-center gap-1.5" title={vivienda.bathroom ? `${vivienda.bathroom} ${bathrooms}` : 'N/A'}>
            <span className="text-sm">🚿</span>
            <span className="font-medium">{vivienda.bathroom ? `${vivienda.bathroom} ${bathrooms}` : 'N/A'}</span>
          </div>
        </div>

        {/* Precio en la parte inferior - Solo visible en desktop */}
        <div className="hidden md:block mt-auto pt-2 border-t border-gray-200">
