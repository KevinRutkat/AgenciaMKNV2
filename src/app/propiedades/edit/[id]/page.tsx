  ...
  const loadPropertyData = async () => {
    try {
      ...
      if (viviendaError) {
        ...
        setSubmitMessage('❌ Error al cargar la propiedad: ' + viviendaError.message);
        return;
      }

      if (!vivienda) {
        console.error('Property not found with ID:', propertyId);
        setSubmitMessage('❌ No se encontró la propiedad con ID: ' + propertyId);
        return;
      }
      ...
      if (vivienda) {
        ...
        const propiedadesCanonicas = propiedadesArray.map((p) => {
          ...
          const match = FEATURES.find(f => f.key === norm || normalizeFeature(f.label) === norm);
      ...
      setSubmitMessage(`❌ Error al cargar los datos de la propiedad: ${error instanceof Error ? error.message : 'Error desconocido'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (loading || isLoading) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-teal-50 to-white flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-teal-600 mx-auto mb-4"></div>
          <p className="text-teal-600">Cargando datos de la propiedad...</p>
        </div>
      </div>
    );
  }
              ...
              <div className="space-y-4">
                ...
                <div className="bg-blue-50 p-3 rounded-lg border border-blue-200">
                  ...
                  <p className="text-xs text-blue-600">
                    ...
                    • Puedes volver a seleccionar más imágenes cuando quieras agregar adicionales<br/>
                    • Las imágenes existentes se mantienen, solo agregas nuevas
                  </p>
                </div>
